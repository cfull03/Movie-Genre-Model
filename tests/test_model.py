...
class TestBuildModel:
    ...
    def test_build_model_default_parameters(self):
        ...
        assert base_estimator.C == 50.0
        assert base_estimator.penalty == 'l2'
        assert base_estimator.solver == 'lbfgs'
        assert base_estimator.max_iter == 2000
    ...
    def test_build_model_custom_parameters(self):
        ...
        model = build_model(C=100.0, penalty='l1', solver='liblinear', max_iter=500)
        base_estimator = model.estimator
        assert base_estimator.C == 100.0
        assert base_estimator.penalty == 'l1'
        assert base_estimator.solver == 'liblinear'
...
class TestGetParams:
    ...
    def test_get_params_extracts_correctly(self):
        ...
        model = build_model(C=75.0, penalty='l2', solver='lbfgs', max_iter=1500)
...
class TestSaveLoadModel:
    ...
    def test_load_model_returns_correct_type(self, sample_model, tmp_path, monkeypatch):
        ...
        assert isinstance(loaded_model.estimator, LogisticRegression)
    ...
    def test_save_and_load_preserves_parameters(self, tmp_path, monkeypatch):
        ...
        original_model = build_model(C=100.0, penalty='l1', solver='liblinear')
        save_model(original_model, 'test_model')
        loaded_model = load_model('test_model')
        
        assert loaded_model.estimator.C == 100.0
        assert loaded_model.estimator.penalty == 'l1'
        assert loaded_model.estimator.solver == 'liblinear'
