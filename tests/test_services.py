...
def mock_service_components():
    """Create mocked pipeline and mlb for testing."""
    mock_classifier = Mock()
    mock_classifier.decision_function.return_value = np.array(
        [[0.8, 0.6, 0.4, 0.3, 0.2]]
    )
...
def configured_service(mock_service_components):
    """Service with mocked pipeline configured."""
    service = PredictionService()
    service.pipeline = mock_service_components["pipeline"]
    service.model = mock_service_components["model"]
    service.mlb = mock_service_components["mlb"]
    service._is_loaded = True
    service._use_pipeline = True
    return service
...
class TestPredictionServiceTopK:
    ...
    def test_top_k_with_threshold(self, mock_service_components):
        ...
        mock_service_components["model"].decision_function.return_value = (
            np.array([[1.4, 1.0, -0.4, -0.8]])
        )
    ...
    def test_multiple_descriptions(self, mock_service_components):
        ...
        mock_service_components["pipeline"].named_steps["tfidf"].transform.return_value = (
            csr_matrix([[1, 2], [3, 4]])
        )
        mock_service_components["pipeline"].named_steps["normalizer"].transform.return_value = (
            csr_matrix([[1, 2], [3, 4]])
        )
        mock_service_components["pipeline"].named_steps["feature_selector"].transform.return_value = (
            csr_matrix([[1, 2], [3, 4]])
        )
        mock_service_components["mlb"].inverse_transform.return_value = [
            ("Action", "Thriller"),
